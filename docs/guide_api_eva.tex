...
Dans un pipeline de traitement, les erreurs passagères (modèle en cours de chargement,
limite de débit dépassée) sont inévitables. Voici un patron de code avec relance automatique :
...
def query_with_retry(messages: list[dict],
    ...
    Attend le chargement du modele si necessaire (503).
    """
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if e.status_code == 503:
                # Modele en cours de chargement — attendre et reessayer
                wait = 30 * attempt
                print(f"Modele en chargement, attente {wait}s "
...
\begin{tabular}{C{1.5cm} L{3.5cm} L{4cm} L{4cm}}
  ...
  \addlinespace
  503 & server\_error & Modèle en cours de chargement & Attendre 30 à 90~s et réessayer \\
  \addlinespace
